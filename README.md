# PulseCloak Node ![pulsecloak-logo](images/pulsecloak-logo-sm.png)

<div align="center">

![ci-matrix](https://github.com/PulseCloak-Project/Node/workflows/ci-matrix/badge.svg)
[![GitHub Release](https://img.shields.io/github/v/release/PulseCloak-Project/Node?display_name=release&color=green)](https://github.com/PulseCloak-Project/Node/releases/latest)
[![Discord](https://badgen.net/badge/icon/discord?icon=discord&label)](https://discord.gg/pulsecloak)

</div>

PulseCloak Node combines the benefits of VPN and Tor technology, to create superior next-generation privacy software, where 
users are rewarded for supporting an uncensored, global Web.  Users gain privacy and anonymity, while helping promote 
Internet Freedom.

## Purpose
**PulseCloak Node** forms the foundation of the PulseCloak Network.

The PulseCloak Network is an open-source decentralized mesh-network (dMN) that allows any user to allocate spare computing resources to make the Internet
...
issue on the PulseCloak Network. It doesn't matter where you live or what content you're accessing, everyone in the world
sees the exact same content.

PulseCloak Node software is what the average user runs to earn $CLOAK utility tokens and dedicate some of their computers' 
resources toward the Network - users can be rewarded with $CLOAK for each time they serve content.

PulseCloak Nodes work together through the mesh network to relay digital content.
...
__Important Note:__ Please remember that at the moment the PulseCloak Node is in development and is not clandestine. Your
...
stage - PulseCloak Network and it's developers are not responsible for any activity, or loss incurred, while using this beta software.

## Source
The PulseCloak project was forked from Substratum's Node project in order to carry on development after Substratum ceased 
operations in October of 2019. In 2021, Substratum's Node repositories were removed from GitHub, so the fork link
with PulseCloak was broken, but all credit for the original idea and the original design belongs to Substratum (and properly attributed through GPLv3 license) 

## Running the PulseCloak Node

### PulseCloak Node Knowledge Base
A [Knowledge Base](https://docs.pulsechaincloak.io/pulsecloak) and testing resources are being refined for users with various 
levels of technical ability.

There you can find further information, guides and configuration examples for running PulseCloak Node from:
- [PulseChainCloak Browser](https://pulsechaincloak.com)
- CLI
- Docker image

If you are interested in testing stages, speak to the team in the official [Discord channel](https://discord.gg/pulsecloak)

### PulseCloak Node Component README.md files
To help navigate the codebase, here are the README.md links for all documented components

- [Blockchain-Service-URL](https://github.com/PulseCloak-Project/Node/blob/master/node/docs/Blockchain-Service.md)
- [sub_lib](https://github.com/PulseCloak-Project/Node/blob/master/node/src/sub_lib/README.md)
- [proxy_server](https://github.com/PulseCloak-Project/Node/blob/master/node/src/proxy_server/README.md)
- [test_utils](https://github.com/PulseCloak-Project/Node/blob/master/node/src/test_utils/README.md)
- [ui_gateway](https://github.com/PulseCloak-Project/Node/blob/master/node/src/ui_gateway/README.md)
- [accountant](https://github.com/PulseCloak-Project/Node/blob/master/node/src/accountant/README.md)
- [proxy_client](https://github.com/PulseCloak-Project/Node/blob/master/node/src/proxy_client/README.md)
- [entry_dns](https://github.com/PulseCloak-Project/Node/blob/master/node/src/entry_dns/README.md)
- [hopper](https://github.com/PulseCloak-Project/Node/blob/master/node/src/hopper/README.md)
- [neighborhood](https://github.com/PulseCloak-Project/Node/blob/master/node/src/neighborhood/README.md)
- [multinode_integration_tests](https://github.com/PulseCloak-Project/Node/blob/master/multinode_integration_tests/tests/README.md)

### Downloading Official Releases

Releases will appear on our GitHub page - click on the badge above for the latest stable beta build, or go to our [Releases page](https://github.com/PulseCloak-Project/Node/releases/latest)

### Downloading the Latest Build

If you want to try out the latest build, go to
[our GitHub Actions continuous integration page](https://github.com/PulseCloak-Project/Node/actions) to see a list of builds.
...
Make a directory somewhere on your system from which you'll run PulseCloak. You'll want to extract one or more files from
`/generated/bin` in the `.zip` file into that directory.

The most important file is `PulseCloakNode`, or `PulseCloakNode.exe` if you're using Windows. Definitely extract that one. It
contains the code for both the PulseCloak Node and the PulseCloak Daemon.

If you're using a graphical user interface for PulseCloak, that's all you'll need. If you're not, you'll probably also want
`pulsecloak`, which is a command-line user interface.
...
Finally, `automap` is a test utility used to check PulseCloak's automatic firewall penetration functionality against your
particular router. Unless you've volunteered to help the PulseCloak dev team run tests, you won't need this.

### Running from the Command Line

These instructions assume you have the PulseCloak Node executable but not the PulseCloak GUI. (If you do, consult the GUI
documentation about starting the Node on our [Testing Guides](https://docs.pulsechaincloak.io/testing-guide)) You'll want to run the PulseCloak programs from wherever you expanded the
`/generated/bin` path from the `.zip` file you downloaded.

There are a number of ways to run the Node, but the way you'll probably want to use is to make sure the PulseCloak Daemon
is started first. If the Daemon is not running in the background already, open a terminal window and start it by typing

`$ sudo nohup ./PulseCloakNode --initialization &`

if you're working in Linux or macOS, or

`$ start /b PulseCloakNode --initialization`
...
If you have no GUI, the simplest way to do this is with the `pulsecloak` command-line user interface. Once you have the 
Daemon running, type

`$ pulsecloak`

at a handy command prompt. To learn how to use `pulsecloak` to set up and start the Node, type `help` at the `pulsecloak>` prompt,
...
#### Supplying Configuration To PulseCloak Daemon

There are four ways to get configuration information into the initialization area of the PulseCloak Daemon on startup. 
...
`pulsecloak` on the command line, either in interactive mode or in noninteractive mode. Keep in mind, though, that command 
...
parameter in the environment by setting the `PCLOAK_CLANDESTINE_PORT` environment variable to `1234`. Note that you need
to remove the initial `--` prefix, convert the name to all uppercase, change hyphens to underscores, and add a `PCLOAK_` 
...
information) and is named `config.toml`. If you leave the configuration file unspecified, this is where PulseCloak Node
will look for it. If it's found, it will be used; if it's not, PulseCloak Node will act as though it had been found but empty.
But if you want to use a different file, specify it either as `--config-file` in the Daemon setup or as `PCLOAK_CONFIG_FILE`
in the environment. If you specify a relative filename, PulseCloak Node will look for the configuration file in the data
directory; if you specify an absolute filename, PulseCloak Node will not use the data directory to find the configuration
file.

The configuration file should be in TOML format. TOML is a competitor to other formats like JSON and YAML, but the
PulseCloak Node uses only scalar settings, not arrays or tables. If you see that Daemon setup accepts a command such
...
can read whatever's in it, whether PulseCloak Node is running or not. Therefore, the configuration file may not be the 
best place to specify sensitive or secret configuration information. (Nothing prevents you from doing this, though, so 
be careful.)

#### Running a Decentralized PulseCloak Node Locally

##### Wallets

In order to run decentralized, PulseCloak Node needs at least an earning wallet (an Ethereum wallet into which other Nodes
...
PulseCloak only ever has to put money into your earning wallet, which means the Node only has to know its address. However,
PulseCloak needs to pay money out of your consuming wallet; therefore the Node needs to know more about that one. Currently,
...
used, in which case the funds you earn will go to PulseCloak instead of to you: so unless you're in a philanthropic mood,
...
PulseCloak never stores the password anywhere on disk, only in memory; so A) you'll need to supply the password every time
...
##### Interactive `pulsecloak` vs Noninteractive `pulsecloak`

The `pulsecloak` command-line interface can run two ways. If you just type

```
$ ./pulsecloak
```

at a command prompt, you'll be shown a `pulsecloak>` prompt, and the system will await a series of `pulsecloak` commands from you.
But if you type something like

```
$ ./pulsecloak setup --log-level debug --clandestine-port 1234
```

then `pulsecloak` will start up, execute the command you gave it, and immediately terminate.

This way, you can use interactive `pulsecloak` to give an impromptu series of commands to the Daemon and/or the Node, or you
can write shell scripts to control the Daemon and/or the Node for special purposes, with those scripts containing
noninteractive `pulsecloak` commands.

##### Daemon vs. Node

The PulseCloak Daemon and the PulseCloak Node are two different programs that share the same binary. If you start that binary with
...
PulseCloak is designed to be very difficult to hack, but it's intended to go up against government-level attackers, so there's
always the possibility that they could have the funding to do something we didn't anticipate. If an attacker figures out
how to hack into a computer running PulseCloak, we think it's very important that at least he doesn't find himself hacked
...
These two requirements led to the development of the PulseCloak Daemon. The Daemon should start up with administrative
...
##### Handy `pulsecloak` Subcommands

`pulsecloak` has quite a few subcommands that you can use, both interactively and noninteractively. The best way to find
...
interactively at a `pulsecloak` prompt.

##### Neighbors

If you're starting the very first PulseCloak Node in a PulseCloak network, then you don't have to tell your Node about
any preexisting network; instead, other people who want to join that PulseCloak Network will have to tell their Nodes
about your Node, which means you'll need to give them your Node descriptor.

Otherwise, you'll need to specify `--neighbors` as part of your `pulsecloak setup` so that your Node will know how to join 
the network that is already in place.

However, if your machine has already recently been on the PulseCloak Network, and you're starting the Node up again, there's 
...
In order to run decentralized, the PulseCloak Node _must_ know the IP address others can use to contact it. Therefore,
you must supply `--ip`.

Your home network is behind your internet provider's router and a public IP address. Other Nodes in the PulseCloak Network
will contact your Node through your public IP address, requiring at least one port to be forwarded on your router. The 
PulseCloak Node Gossip protocol "gossips" to other Nodes the clandestine port you are listening on, and it is that port 
...
[in the neighborhood_subproject](https://github.com/PulseCloak-Project/Node/tree/master/node/src/neighborhood).


### Terminating a PulseCloak Node

Terminating a PulseCloak Node may be more of a process than you expect. There are three things to consider.

The most obvious is the PulseCloak Node itself. Terminating that is easy: just send it a shutdown message, with the GUI if 
you have it, or with `pulsecloak` like this:
...
PulseCloak Node, like any other piece of software, can encounter obstacles it cannot overcome in the process of trying
...
connections. Let's look at how PulseCloak Node deals with certain kinds of errors.

### HTTP

An insecure HTTP connection is one that is based on a URL that begins with `http://` (as opposed to `https://`). The
fact that it is insecure means that PulseCloak Node (and every other process that handles the data) can intrude on the
...
When errors occur, this is very useful for PulseCloak Node. If you request something from an HTTP server (and for some
reason PulseCloak Node cannot relay your request to that server) or cannot relay the response from the server back to
you, PulseCloak Node will instead impersonate the server and create a counterfeit response describing the error. This will be displayed
to you instead of the server response it can't give you. (Don't worry: PulseCloak Node's impersonation of the server
...
connection is set up between your browser and a server, PulseCloak Node cannot understand a single bit of the dataflow,
and it cannot modify a single bit of it without your browser throwing red alerts and refusing to show you the modified
data. This is good for you and your privacy, but it doesn't make it easy for PulseCloak Node to communicate with you
via the browser.
...
been established, there's a little PulseCloak Node can do. Specifically, it can inject what's called a TLS Alert into the
stream of data, as long as it is injected very early. This TLS Alert has a single byte that PulseCloak Node can use to tell
you about problems it has relaying your data. There are a number of predefined values this byte can take on, and
PulseCloak Node has to pick one of these values: it can't make up its own.
...
PulseCloak Node sent a TLS Alert, you'll see it there.

Since the concerns of the PulseCloak Node aren't precisely the same as the concerns of a TLS endpoint, the correspondence
can't always be made exact, so here are some specific TLS Alert values that PulseCloak Node produces in specific 
situations.

* Routing Failure - `internal_error`: If your Node is not yet "warmed up" enough in the PulseCloak Network to see a
...
strictly speaking, it is. If this happens, just wait awhile for your Node and the PulseCloak Network to Gossip with
...
can't give you the correct IP address for the server name you're seeking. PulseCloak Node captures the DNS queries your
browser makes and forwards them across the PulseCloak Network to some other Node that hopefully has access to a
...
PulseCloak Node guessed wrong, and the exit Node to which it forwarded your DNS query is also handicapped by a censored DNS
and can't find it either. In either case, PulseCloak Node will send your browser a TLS `unrecognized_name` alert, which
your browser will probably present to you as some form of can't-find-host error. If you reload the page, PulseCloak Node
...
PulseCloak Node doesn't reliably build on 32-bit Windows due to issues with the build tools for that platform. We 
...
Copyright (c) 2019-2024, PulseCloak Network

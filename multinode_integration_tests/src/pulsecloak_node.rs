...
impl From<&dyn PulseCloakNode> for NodeReference {
...
pub trait PulseCloakNode: Any {
    // This is the name of the Docker container on which this PulseCloakNode will run.
    fn name(&self) -> &str;
    ...
    // If this PulseCloakNode has a main CryptDENull instead of a CryptDEReal, you can get it here.
    fn main_cryptde_null(&self) -> Option<&CryptDENull>;
    // If this PulseCloakNode has an alias CryptDENull instead of a CryptDEReal, you can get it here.
    fn alias_cryptde_null(&self) -> Option<&CryptDENull>;
    // The CryptDE that can be used for signing for this Node, if any. (None if it's a PulseCloakRealNode with a CryptDEReal.)
    fn signing_cryptde(&self) -> Option<&dyn CryptDE>;
    // A reference to this PulseCloakNode's main public key.
    fn main_public_key(&self) -> &PublicKey;
    // A reference to this PulseCloakNode's alias public key.
...
pub struct PulseCloakNodeUtils {}

impl PulseCloakNodeUtils {
    ...
    pub fn assert_node_wrote_log_containing(name: &str, pattern: &str, timeout: Duration) {
        ...
        while Instant::now() < time_limit {
            entire_log = PulseCloakNodeUtils::retrieve_logs(name);
...
mod tests {
    ...
    fn node_reference_from_str_fails_if_there_are_not_two_fields_to_divide_to() {
        let string = String::from("pulsecloak://two@fields@nope");

        let result = NodeReference::from_str(string.as_str());

        assert_eq! (result, Err (String::from ("Either '@' delimiter position or format of node address is wrong. Should be 'pulsecloak://<chain identifier>:<public key>@<node address>', not 'pulsecloak://two@fields@nope'\nNodeAddr should be expressed as '<IP address>:<port>/<port>/...', probably not as 'fields@nope'")));
    }

    #[test]
    fn node_reference_from_str_fails_if_key_is_not_valid_base64() {
        let string = String::from("pulsecloak://dev:;;;@12.34.56.78:1234/2345");
    ...
    fn node_reference_from_str_fails_if_ip_address_is_not_valid() {
        ...
        let string = format!("pulsecloak://dev:{}@blippy:1234/2345", key);
        ...
        assert_eq!(
            result,
            Err(String::from(
                "Either '@' delimiter position or format of node address is wrong. Should be 'pulsecloak://<chain identifier>:<public key>@<node address>', not 'pulsecloak://dev:Qm9vZ2E@blippy:1234/2345'\nNodeAddr should be expressed as '<IP address>:<port>/<port>/...', probably not as 'blippy:1234/2345'"
            ))
        );
    }
    ...
    fn node_reference_from_str_fails_if_a_port_number_is_not_valid() {
        let key = PublicKey::new(&b"Booga"[..]);
        let string = format!("pulsecloak://dev:{}@12.34.56.78:weeble/frud", key);

        let result = NodeReference::from_str(string.as_str());

        assert_eq! (result, Err (String::from("Either '@' delimiter position or format of node address is wrong. Should be 'pulsecloak://<chain identifier>:<public key>@<node address>', not 'pulsecloak://dev:Qm9vZ2E@12.34.56.78:weeble/frud'\nNodeAddr should be expressed as '<IP address>:<port>/<port>/...', probably not as '12.34.56.78:weeble/frud'")));
    }

    #[test]
    fn node_reference_from_str_fails_if_a_port_number_is_too_big() {
        let key = PublicKey::new(&b"Booga"[..]);
        let string = format!("pulsecloak://dev:{}@12.34.56.78:1234/65536", key);
    ...
    fn node_reference_from_str_happy_path() {
        let key = PublicKey::new(&b"Booga"[..]);
        let string = format!("pulsecloak://dev:{}@12.34.56.78:1234/2345", key);
    ...
    fn node_reference_from_str_complains_about_slash_in_the_key() {
        let result = NodeReference::from_str(
            "pulsecloak://dev:abJ5XvhVbmVyGejkYUkmftF09pmGZGKg/PzRNnWQxFw@12.23.34.45:5678",
        );
    ...
    fn node_reference_from_str_complains_about_plus_in_the_key() {
        let result = NodeReference::from_str(
            "pulsecloak://dev:abJ5XvhVbmVy+GejkYUmftF09pmGZGKgkPzRNnWQxFw@12.23.34.45:5678",
        );
    ...
    fn node_reference_from_string_works_if_there_are_no_ports() {
        let key = PublicKey::new(&b"Booga"[..]);
        let string = format!("pulsecloak://dev:{}@12.34.56.78:", key);
    ...
    fn node_reference_can_display_itself() {
        ...
        assert_eq!(
            result,
            String::from("pulsecloak://dev:Qm9vZ2E@12.34.56.78:1234/5678")
        );
    }

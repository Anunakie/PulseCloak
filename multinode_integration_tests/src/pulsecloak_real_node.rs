...
use crate::pulsecloak_node::NodeReference;
use crate::pulsecloak_node::PortSelector;
use crate::pulsecloak_node_client::PulseCloakNodeClient;
use crate::pulsecloak_node_server::PulseCloakNodeServer;
use crate::pulsecloak_node_ui_client::PulseCloakNodeUIClient;
use bip39::{Language, Mnemonic, Seed};
use log::Level;
use pulsecloak_lib::blockchains::chains::Chain;
use pulsecloak_lib::constants::{CURRENT_LOGFILE_NAME, DEFAULT_UI_PORT};
use pulsecloak_lib::test_utils::utils::TEST_DEFAULT_MULTINODE_CHAIN;
use pulsecloak_lib::utils::{localhost, to_string};
use pulsecloak_lib::utils::{DEFAULT_CONSUMING_DERIVATION_PATH, DEFAULT_EARNING_DERIVATION_PATH};
...
#[derive(Clone, Debug)]
pub struct PulseCloakRealNode {
    guts: Rc<PulseCloakRealNodeGuts>,
}

impl PulseCloakNode for PulseCloakRealNode {
    fn name(&self) -> &str {
        &self.guts.name
    }
    ...
    fn socket_addr(&self, port_selector: PortSelector) -> SocketAddr {
        PulseCloakNodeUtils::socket_addr(&self.node_addr(), port_selector, self.name())
    }
    ...
    fn country_code_opt(&self) -> Option<String> {
        PulseCloakNodeUtils::derive_country_code_opt(&self.node_addr())
    }
}

impl PulseCloakRealNode {
    ...
    pub fn start_with(
        ...
        PulseCloakNodeUtils::clean_up_existing_container(name);
        let real_startup_config = match startup_config.ip_info {
            ...
            LocalIpInfo::DistributedKnown(ip_addr) => panic!(
                "Can't pre-specify the IP address of a PulseCloakRealNode: {}",
                ip_addr
            ),
        };
        let root_dir = match host_node_parent_dir {
            Some(dir) => dir,
            None => PulseCloakNodeUtils::find_project_root(),
        };
        ...
        let guts = Rc::new(PulseCloakRealNodeGuts {
    ...
    pub fn kill_node(&self) {
        let _ =
            Self::exec_command_on_container_and_wait(&self.guts.name, vec!["pkill", "PulseCloakNode"]);
    }
    ...
    pub fn restart_node(&self, startup_config: NodeStartupConfig) {
        ...
        Self::exec_command_on_container_and_detach(&self.guts.name, bash_command_parts)
            .expect("Couldn't start PulseCloakNode");
    }
    ...
    pub fn make_client(&self, port: u16, timeout_millis: u64) -> PulseCloakNodeClient {
        let socket_addr = SocketAddr::new(self.ip_address(), port);
        PulseCloakNodeClient::new(socket_addr, timeout_millis)
    }

    pub fn make_server(&self, port: u16) -> PulseCloakNodeServer {
        PulseCloakNodeServer::new(port)
    }

    pub fn make_ui(&self, port: u16) -> PulseCloakNodeUIClient {
        PulseCloakNodeUIClient::new(SocketAddr::new(self.guts.container_ip, port))
    }
    ...
    fn establish_wallet_info(name: &str, startup_config: &NodeStartupConfig) {
        ...
        let node_command = format!("/node_root/node/PulseCloakNode {}", args);
        let mut bash_command_parts = vec!["/bin/bash", "-c"];
        bash_command_parts.extend(vec![node_command.as_str()]);
    ...
    fn create_node_command(node_args: Vec<String>, startup_config: NodeStartupConfig) -> String {
        let mut node_command_parts: Vec<String> = match startup_config.memory_opt {
            Some(kbytes) => vec![format!(
                "ulimit -v {} -m {} && /node_root/node/PulseCloakNode",
                kbytes, kbytes
            )],
            None => vec!["/node_root/node/PulseCloakNode".to_string()],
        };
    ...
    fn do_docker_run(
        ...
        let test_runner_node_home_dir =
            Self::node_home_dir(&PulseCloakNodeUtils::find_project_root(), container_name_ref);
    ...
    fn descriptor_regex() -> Regex {
        Regex::new(r"PulseCloak Node local descriptor: (pulsecloak://.+:.+@[\d.]*:[\d,]*)").unwrap()
    }
...
impl Drop for PulseCloakRealNodeGuts {
    fn drop(&mut self) {
        PulseCloakNodeUtils::stop(self.name.as_str())
    }
}
...
mod tests {
    ...
    fn regex_captures_descriptor() {
        let text = "scajcbakbcskjbcbackjbb PulseCloak Node local descriptor: pulsecloak://dev:BrrLUksswnE8GOQQMpwcAjk2hOX4HEmaTcBloBpPuE0@: jajca[cjscpajpojsc";
        let regex = PulseCloakRealNode::descriptor_regex();
        ...
        assert_eq!(
            result.as_str(),
            "pulsecloak://dev:BrrLUksswnE8GOQQMpwcAjk2hOX4HEmaTcBloBpPuE0@:"
        )
    }
}

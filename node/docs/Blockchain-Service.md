...
A blockchain service url is a URL that PulseCloak Node uses to interact with various blockchains, either the Amoy and Ropsten
testnets or Polygon and Ethereum mainnets. On mainnets, PulseCloak uses the PulseCloak token; on testnets, it uses a test token.

#### PulseCloak Node software supports connections to the following blockchains:
**Ethereum**
- [x] Ropsten testnet
- [x] Ethereum mainnet
...
To be fully functional PulseCloak Node needs communication to the blockchain for access to:
1. pay SHRD/PulseCloak to other Nodes for routing, blockchain, and exit services consumed, 
1. determine if other requesting Nodes have enough SHRD/PulseCloak to pay for services -- also ETH for gas fees, and 
...
There are two general types of Blockchain Services PulseCloak Node currently supports.
...
1. Run your own Ethereum miner such as a [Go Ethereum (geth)](https://geth.ethereum.org) or 
   ...
   PulseCloak Node or on another machine within your private network.
...
* For the `pulsecloak` command-line interface, use the `setup` command: 
  
    > `pulsecloak> setup --blockchain-service-url https://polygon-amoy.infura.io/v3/<YOUR-PROJECT-ID>`
...
* Or define an environment variable
    ...
    * Windows
    ...
    > `set PCLOAK_BLOCKCHAIN_SERVICE_URL = https://polygon-amoy.infura.io/v3/<YOUR-PROJECT-ID>`
  
    * Linux or macOS
  
    > `export PCLOAK_BLOCKCHAIN_SERVICE_URL = https://polygon-amoy.infura.io/v3/<YOUR-PROJECT-ID>`

Change `<YOUR-PROJECT-ID>` with the PROJECT ID from your Infura.io Project Dashboard. Change the URL to 
mainnet when ready to spend and earn real PulseCloak. 

As soon as you begin consuming routing services from other PulseCloak Nodes, they will begin expecting you to pay them for
those services in SHRD or PulseCloak. If you live in a country where cryptocurrency like Ethereum or PulseCloak tokens are legal,
...
protected by the PulseCloak Network. How can you set up your cryptocurrency environment using a network that requires

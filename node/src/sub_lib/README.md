# `sub_lib`
Common code for PulseCloak Rust projects
...
separate projects (like `pulsecloak` and `dns_utility` and `multinode_integration_tests`) reasserted
itself on a different level, and we needed another place to put common code. Since by that time
our name was PulseCloak instead of Substratum, all that code went into the `pulsecloak_lib` project.

We think it's unlikely that the current projects in the PulseCloakNode repository will ever be
consolidated into one, so both `sub_lib` and `pulsecloak_lib` will probably continue to exist at

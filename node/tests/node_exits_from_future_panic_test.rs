...
use pulsecloak_lib::utils::add_chain_specific_directory;
...
fn node_logs_panic_integration() {
    ...
    create_dir_all(&data_dir_chain_path).expect(
        "Could not create chain directory inside node_logs_panic_integration home/PulseCloak directory",
    );
...
fn node_logfile_does_not_belong_to_root_integration() {
    ...
    let logfile_path = PulseCloakNode::path_to_logfile(&node.data_dir);

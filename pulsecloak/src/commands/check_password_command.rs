...
use pulsecloak_lib::messages::{UiCheckPasswordRequest, UiCheckPasswordResponse};
...
mod tests {
    ...
    use pulsecloak_lib::messages::{ToMessageBody, UiCheckPasswordRequest, UiCheckPasswordResponse};
    ...
    fn testing_command_factory_with_bad_command() {
        ...
        match result {
            Err(CommandFactoryError::CommandSyntax(msg)) => {
                // Note: when run with PulseCloak/Node/ci/all.sh, msg contains escape sequences for color.
                assert_eq!(

...
use pulsecloak_lib::messages::{UiConfigurationRequest, UiConfigurationResponse};
    ...
    fn execute(&self, context: &mut dyn CommandContext) -> Result<(), CommandError> {
        ...
        match output {
            ...
            Err(Payload(code, message)) if code == NODE_NOT_RUNNING_ERROR => {
                short_writeln!(
                    context.stderr(),
                    "PulseCloakNode is not running; therefore its configuration cannot be displayed."
                );

...
use pulsecloak_lib::messages::{UiDescriptorRequest, UiDescriptorResponse};
...
const DESCRIPTOR_SUBCOMMAND_ABOUT: &str =
    "Displays the Node descriptor of the running PulseCloakNode. Only valid if Node is already running.";
    ...
    fn execute(&self, context: &mut dyn CommandContext) -> Result<(), CommandError> {
        ...
        match output {
            ...
            Err(Payload(code, message)) if code == NODE_NOT_RUNNING_ERROR => {
                short_writeln!(
                    context.stderr(),
                    "PulseCloakNode is not running; therefore its descriptor cannot be displayed."
                );
...
mod tests {
    ...
    use pulsecloak_lib::messages::{ToMessageBody, UiDescriptorRequest, UiDescriptorResponse};

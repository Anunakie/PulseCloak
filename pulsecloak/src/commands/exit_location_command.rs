// Copyright (c) 2025, PulseCloak (https://pulsechaincloak.io) and/or its affiliates. All rights reserved.
...
use pulsecloak_lib::constants::EXIT_COUNTRY_MISSING_COUNTRIES_ERROR;
use pulsecloak_lib::exit_locations::ExitLocationSet;
use pulsecloak_lib::messages::{CountryGroups, UiSetExitLocationRequest, UiSetExitLocationResponse};
use pulsecloak_lib::shared_schema::common_validators;
use pulsecloak_lib::{as_any_ref_in_trait_impl, short_writeln};
use std::fmt::Debug;
...
const EXIT_LOCATION_ABOUT: &str =
    ...
    Here are some example commands:\n\
        pulsecloak> exit-location --show-countries    // show all country codes available for exit in Database\n\
        pulsecloak> exit-location                     // disable exit-location preferences\n\
        pulsecloak> exit-location --fallback-routing  // disable exit-location preferences\n\
        pulsecloak> exit-location --country-codes \"CZ,PL|SK\" --fallback-routing \n\t// fallback-routing is ON, \"CZ\" and \"PL\" countries have same priority \"1\", \"SK\" has priority \"2\"\n\
        pulsecloak> exit-location --country-codes \"CZ|SK\"       \n\t// fallback-routing is OFF, \"CZ\" and \"SK\" countries have different priority\n";

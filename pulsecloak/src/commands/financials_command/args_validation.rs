...
use crate::commands::financials_command::parsing_and_value_dressing::restricted::{
    parse_pulsecloak_range_to_gwei, parse_time_params,
};
use clap::{App, Arg, ArgGroup, SubCommand};
use pulsecloak_lib::shared_schema::common_validators::validate_non_zero_u16;
use num::CheckedMul;
...
const PAYABLE_ARG_HELP: &str = "Enables querying payable records by two specified ranges, one for the age in seconds and another for the balance in PulseCloaks (use the decimal notation to achieve the desired gwei precision). \
...
const RECEIVABLE_ARG_HELP: &str = "Enables querying receivable records by two specified ranges, one for the age in seconds and another for the balance in PulseCloaks (use the decimal notation to achieve the desired gwei precision). \
 The correct format consists of two ranges separated by | as in the example <MIN-AGE>-<MAX-AGE>|<MIN-BALANCE>-<MAX-BALANCE>. Leaving out <MAX-BALANCE>, including the preceding hyphen, will default to maximum (2^64 - 1). If this \
  parameter is being set in the non-interactive mode, the value needs to be enclosed in quotes (single or double).";
...
const GWEI_HELP: &str =
    "Orders money values rendering in gwei of PulseCloak instead of whole PulseCloaks as the default.";
    ...
    <i64 as TryFrom<N>>::Error: Debug,
{
    ...
    let (min_amount, max_amount, _, _): (N, N, _, _) = parse_pulsecloak_range_to_gwei(balance_range)?;
...
mod tests {
    ...
    fn constants_have_correct_values() {
        ...
        assert_eq!(PAYABLE_ARG_HELP, "Enables querying payable records by two specified ranges, one for the age in seconds and another for the balance in PulseCloaks (use the decimal notation to achieve the desired gwei precision). \
        ...
        assert_eq!(RECEIVABLE_ARG_HELP, "Enables querying receivable records by two specified ranges, one for the age in seconds and another for the balance in PulseCloaks (use the decimal notation to achieve the desired gwei precision). \
            The correct format consists of two ranges separated by | as in the example <MIN-AGE>-<MAX-AGE>|<MIN-BALANCE>-<MAX-BALANCE>. Leaving out <MAX-BALANCE>, including the preceding hyphen, will default to maximum (2^64 - 1). \
            If this parameter is being set in the non-interactive mode, the value needs to be enclosed in quotes (single or double).");
    ...
    #[test]
    fn validate_two_ranges_also_integers_are_acceptable_for_pulsecloaks_range() {
    ...
    #[test]
    fn validate_two_ranges_one_side_negative_range_is_acceptable_for_pulsecloaks_range() {
        let result = validate_two_ranges::<i64>("454-2000|-2000-30000".to_string());
    ...
    #[test]
    fn validate_two_ranges_both_side_negative_range_is_acceptable_for_pulsecloaks_range() {
    ...
    #[test]
    fn validate_two_ranges_both_values_the_same_for_pulsecloaks() {
    ...
    #[test]
    fn validate_two_ranges_second_value_smaller_than_the_first_for_pulsecloaks_but_not_in_decimals() {
    ...
    #[test]
    fn validate_two_ranges_second_value_smaller_than_the_first_for_pulsecloaks_in_decimals() {

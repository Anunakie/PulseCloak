...
use crate::commands::financials_command::parsing_and_value_dressing::restricted::{
    parse_pulsecloak_range_to_gwei, parse_time_params, split_time_range,
};
        ...
        let parse_params = |age_min: &str,
         ...
         -> ((u64, u64), (N, N, String, String)) {
            (
                ...
                parse_pulsecloak_range_to_gwei(money_range).expect("blew up after validation"),
            )
        };
...
mod tests {
    ...
    #[test]
    fn supplied_big_pulsecloak_values_are_not_fatal_for_non_decimal_values() {
        let factory = CommandFactoryReal::new();
        let result = factory
            .make(&slice_of_strs_to_vec_of_strings(&[
                "financials",
        ...
        assert!(err.contains("Amount bigger than the PulseCloak total supply: 15800000800045"))
    }

    #[test]
    fn supplied_big_pulsecloak_values_are_not_fatal_for_decimal_values() {
        let factory = CommandFactoryReal::new();
        let result = factory
            .make(&slice_of_strs_to_vec_of_strings(&[
                "financials",
        ...
        assert!(
            err.contains("Amount bigger than the PulseCloak total supply: 480045454455.00"),
            "{}",
            err
        )
    }
    ...
    fn default_financials_command_happy_path() {
        ...
        assert_eq!(
            ...
            "\
                \n\
                Financial status totals in PulseCloak\n\
                \n\
    ...
    #[test]
    fn financials_command_stats_and_top_records_default_units_as_pulsecloak() {
        let transact_params_arc = Arc::new(Mutex::new(vec![]));
        ...
        assert_eq!(stdout_arc.lock().unwrap().get_string(),
                ...
                Payable\n\
                \n\
                #   Wallet                                       Age [s]         Balance [PulseCloak]   Pending tx                                                        \n\
                ...
                Receivable\n\
                \n\
                #   Wallet                                       Age [s]   Balance [PulseCloak]\n\
    ...
    #[test]
    fn financials_command_stats_and_custom_query_demanded_default_units_as_pulsecloak() {
        let transact_params_arc = Arc::new(Mutex::new(vec![]));
        ...
        assert_eq!(stdout_arc.lock().unwrap().get_string(),
                ...
                \n\
                \n\
                Specific payable query: 0-350000 sec 0.005-9 PulseCloak\n\
                \n\
                #   Wallet                                       Age [s]   Balance [PulseCloak]   Pending tx                                                        \n\
                ...
                \n\
                \n\
                Specific receivable query: 5000-10000 sec 0.003-5.60007 PulseCloak\n\
                \n\
                #   Wallet                                       Age [s]   Balance [PulseCloak]\n\
                \n\
                No records found\n"
    ...
    fn financials_command_statistics_and_custom_query_with_gwei_precision() {
        ...
        assert_eq!(stdout_arc.lock().unwrap().get_string(), "\
                ...
                \n\
                \n\
                Specific receivable query: 5000-10000 sec 0.000004-0.455 PulseCloak\n\
                \n\
                #   Wallet                                       Age [s]   Balance [gwei]\n\
                \n\
                No records found\n");
    ...
    fn custom_query_balance_range_can_be_shorthanded() {
        ...
        assert_eq!(stdout_arc.lock().unwrap().get_string(),
                   "\n\
            Specific payable query: 0-350000 sec 5-UNLIMITED PulseCloak\n\
            \n\
            #   Wallet                                       Age [s]   Balance [PulseCloak]   Pending tx                                                        \n\
            ...
            \n\
            \n\
            Specific receivable query: 5000-10000 sec 0.8-UNLIMITED PulseCloak\n\
            \n\
            #   Wallet                                       Age [s]   Balance [PulseCloak]\n\
...
|
|
|Specific payable query: 0-400000 sec 355-6000 PulseCloak
|
|#   Wallet                                       Age [s]   Balance [PulseCloak]   Pending tx
|
|No records found
...
|
|
|Specific receivable query: 40000-80000 sec 111-10000 PulseCloak
|
|#   Wallet                                       Age [s]   Balance [PulseCloak]
|
|No records found"
    ...
    fn financials_command_only_payable_demanded() {
        ...
        assert_eq!(
            stdout_arc.lock().unwrap().get_string(),
            "\n\
                Specific payable query: 3000-40000 sec 88-1000 PulseCloak\n\
                \n\
                #   Wallet                                       Age [s]     Balance [PulseCloak]   Pending tx                                                        \n\
    ...
    fn financials_command_only_receivable_demanded() {
        ...
        assert_eq!(
            stdout_arc.lock().unwrap().get_string(),
            "\n\
                Specific receivable query: 3000-40000 sec 66-980 PulseCloak\n\
                \n\
                #   Wallet                                       Age [s]     Balance [gwei]\n\

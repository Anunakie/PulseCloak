...
pub(in crate::commands::financials_command) mod restricted {
    ...
    use pulsecloak_lib::constants::{PCLOAK_TOTAL_SUPPLY, WEIS_IN_GWEI};
    ...
    pub fn convert_pulsecloak_from_gwei_and_dress_well(balance_gwei: i64) -> String {
        const MASK_FOR_NON_SIGNIFICANT_DIGITS: i64 = 10_000_000;
        let balance_pulsecloak_int = (balance_gwei / WEIS_IN_GWEI as i64).abs();
        let balance_pulsecloak_frac = (balance_gwei % WEIS_IN_GWEI as i64).abs();
        let balance_pulsecloak_frac_trunc = balance_pulsecloak_frac / MASK_FOR_NON_SIGNIFICANT_DIGITS;
        match (
            (balance_pulsecloak_int == 0) && (balance_pulsecloak_frac_trunc == 0),
            balance_gwei >= 0,
        ) {
            ...
            _ => {
                format!(
                    ...
                    balance_pulsecloak_int.separate_with_commas(),
                    balance_pulsecloak_frac_trunc
                )
            }
        }
    }
    ...
    pub fn parse_pulsecloak_range_to_gwei<N>(range: &str) -> Result<(N, N, String, String), String>
    where
        ...
        <i64 as TryFrom<N>>::Error: Debug,
    {
        let regex = Regex::new(r#"^((-?\d+\.?\d*)\s*-\s*(-?\d+\.?\d*))|(-?\d+\.?\d*)$"#)
            .expect("wrong regex");
        let (first, second_opt) = extract_individual_pulsecloak_values(range, regex)?;
        ...
        u64: TryFrom<N>,
    {
        fn error_msg<N: Separable>(
            ...
            .collect::<Vec<String>>();
            format!(
                "Supplied value of {} gwei overflows the tech limits. You probably want one between {} and {} PulseCloak", numbers[0], numbers[1], numbers[2]
            )
        }
            ...
            match (e.kind(), minus_sign_regex.is_match(str_gwei)) {
                (IntErrorKind::NegOverflow | IntErrorKind::PosOverflow, _) => {
                    if type_name::<N>() == type_name::<u64>() {
                        error_msg(str_gwei, 0, PCLOAK_TOTAL_SUPPLY)
                    } else {
                        error_msg(
                            str_gwei,
                            -(PCLOAK_TOTAL_SUPPLY as i64),
                            PCLOAK_TOTAL_SUPPLY as i64,
                        )
                    }
                }
            ...
            Ok(int) => match u64::try_from(int) {
                Ok(int_as_u64) => {
                    if int_as_u64 <= i64::MAX as u64 {
                        ...
                        Err(error_msg(str_gwei, 0, PCLOAK_TOTAL_SUPPLY))
                    }
                }
                Err(_) => {
    ...
    pub(super) fn extract_individual_pulsecloak_values(
        pulsecloak_in_range_str: &str,
        pulsecloak_values_in_range_regex: Regex,
        ...
        match pulsecloak_values_in_range_regex
            .captures(pulsecloak_in_range_str)
            .map(handle_captures)
        {
        ...
        <i64 as TryFrom<N>>::Error: Debug,
    {
        ...
        let decimal_shift_to_wei = |parse_result: N, exponent: u32| {
            parse_result
                ...
                .ok_or_else(|| {
                    format!(
                        "Amount bigger than the PulseCloak total supply: {}, total supply: {}",
                        num, PCLOAK_TOTAL_SUPPLY
                    )
                })
        };
...
mod tests {
    use crate::commands::financials_command::parsing_and_value_dressing::restricted::{
        convert_pulsecloak_from_gwei_and_dress_well, extract_individual_pulsecloak_values,
    ...
    #[test]
    fn convert_pulsecloak_from_gwei_and_dress_well_handles_values_smaller_than_one_hundredth_of_pulsecloak_and_bigger_than_zero(
    ) {
        let gwei: i64 = 9999999;

        let result = convert_pulsecloak_from_gwei_and_dress_well(gwei);

        assert_eq!(result, "< 0.01")
    }

    #[test]
    fn convert_pulsecloak_from_gwei_and_dress_well_handles_values_bigger_than_minus_one_hundredth_of_pulsecloak_and_smaller_than_zero(
    ) {
        let gwei: i64 = -9999999;
    ...
    #[test]
    fn convert_pulsecloak_from_gwei_and_dress_well_handles_positive_number() {
    ...
    #[test]
    fn convert_pulsecloak_from_gwei_and_dress_well_handles_negative_number() {
    ...
    fn parse_integer_overflow_indicates_too_big_number_supplied_for_i64() {
        ...
        assert_eq!(err_msg_i64.unwrap_err(), "Supplied value of 9,223,372,036,854,775,808 gwei overflows the tech limits. You probably want one between -37,500,000 and 37,500,000 PulseCloak");

        let err_msg_i64: Result<i64, String> =
            parse_integer_within_limits(&(i64::MIN as i128 - 1).to_string());

        assert_eq!(err_msg_i64.unwrap_err(), "Supplied value of -9,223,372,036,854,775,809 gwei overflows the tech limits. You probably want one between -37,500,000 and 37,500,000 PulseCloak")
    }
    ...
    fn parse_integer_overflow_indicates_too_big_number_supplied_for_u64() {
        ...
        assert_eq!(err_msg_u64.unwrap_err(), "Supplied value of 9,223,372,036,854,775,808 gwei overflows the tech limits. You probably want one between 0 and 37,500,000 PulseCloak");

        let err_msg_u64: Result<u64, String> = parse_integer_within_limits("-1");

        assert_eq!(err_msg_u64.unwrap_err(), "Supplied value of -1 gwei overflows the tech limits. You probably want one between 0 and 37,500,000 PulseCloak")
    }
    ...
    fn u64_detect_minus_sign_error_with_different_white_spaces_around() {
        ["- 5", "   -8", " - 1"].into_iter().for_each(|example|{
            ...
            assert_eq!(err_msg.unwrap_err(), format!("Supplied value of {} gwei overflows the tech limits. You probably want one between 0 and 37,500,000 PulseCloak", example))
        })
    }

    #[test]
    fn i64_interpretation_capabilities_are_good_enough_for_pulsecloak_total_supply_in_gwei() {
        let _: i64 = (PCLOAK_TOTAL_SUPPLY * WEIS_IN_GWEI as u64)
            .try_into()
            .unwrap();
    }
    ...
    fn extract_individual_pulsecloak_values_regex_is_wrong() {
        let regex = Regex::new("(abc)?(def)?(ghi)").unwrap();

        let _ = extract_individual_pulsecloak_values("ghi", regex);
    }

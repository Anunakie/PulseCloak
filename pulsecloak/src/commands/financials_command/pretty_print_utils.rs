...
pub(in crate::commands::financials_command) mod restricted {
    ...
    use crate::commands::financials_command::parsing_and_value_dressing::restricted::{
        convert_pulsecloak_from_gwei_and_dress_well, neaten_users_writing_if_possible,
    };
    use crate::commands::financials_command::FinancialsCommand;
    use pulsecloak_lib::constants::WALLET_ADDRESS_LENGTH;
    use pulsecloak_lib::messages::{UiPayableAccount, UiReceivableAccount};
    ...
    pub fn financial_status_totals_title(stdout: &mut dyn Write, is_gwei: bool) {
        short_writeln!(
            ...
            &gwei_or_pulsecloak_balance(is_gwei)[9..13]
        );
    }
        ...
        <i64 as TryFrom<N>>::Error: Debug,
    {
        if should_stay_gwei {
            ...
            convert_pulsecloak_from_gwei_and_dress_well(gwei_as_i64)
        }
    }
    ...
    fn gwei_or_pulsecloak_balance(is_gwei: bool) -> String {
        format!("Balance {}", gwei_or_pulsecloak_units(is_gwei))
    }

    fn gwei_or_pulsecloak_units(is_gwei: bool) -> &'static str {
        if is_gwei {
            "[gwei]"
        } else {
            "[PulseCloak]"
    ...
    fn prepare_headings_of_records(is_gwei: bool) -> (HeadingsHolder, HeadingsHolder) {
        ...
        let balance = gwei_or_pulsecloak_balance(is_gwei);
        (
            HeadingsHolder {

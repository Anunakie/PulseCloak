...
use pulsecloak_lib::constants::NODE_NOT_RUNNING_ERROR;
use pulsecloak_lib::messages::{UiShutdownRequest, UiShutdownResponse};
...
const SHUTDOWN_SUBCOMMAND_ABOUT: &str =
    "Shuts down the running PulseCloakNode. Only valid if Node is already running.";
    ...
    fn execute(&self, context: &mut dyn CommandContext) -> Result<(), CommandError> {
        ...
        match output {
            ...
            Err(ConnectionProblem(_)) => {
                short_writeln!(
                    context.stdout(),
                    "PulseCloakNode was instructed to shut down and has broken its connection"
                );
                return Ok(());
            }
            Err(Transmission(_)) => {
            ...
            Err(Payload(code, message)) if code == NODE_NOT_RUNNING_ERROR => {
                short_writeln!(
                    context.stderr(),
                    "PulseCloakNode is not running; therefore it cannot be shut down"
                );
        ...
        match context.active_port() {
            None => {
                short_writeln!(
                    context.stdout(),
                    "PulseCloakNode was instructed to shut down and has stopped answering; but the Daemon seems to be down as well"
                );
                Ok(())
            }
            Some(active_port) => {
                if self.shutdown_awaiter.wait(
                    ...
                    short_writeln!(
                        context.stdout(),
                        "PulseCloakNode was instructed to shut down and has stopped answering"
                    );
                    Ok(())
                } else {
                    short_writeln!(
                        context.stderr(),
                        "PulseCloakNode ignored the instruction to shut down and is still running"
                    );

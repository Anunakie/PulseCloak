...
use pulsecloak_lib::messages::{UiStartOrder, UiStartResponse};
...
const START_SUBCOMMAND_ABOUT: &str =
    "Starts a PulseCloakNode with the parameters that have been established by 'setup.' \
    ...
    fn execute(&self, context: &mut dyn CommandContext) -> Result<(), CommandError> {
        ...
        match result {
            Ok(response) => {
                short_writeln!(
                    context.stdout(),
                    "PulseCloakNode successfully started in process {} on port {}",
...
mod tests {
    ...
    fn start_command_happy_path() {
        ...
        assert_eq!(
            ...
            "PulseCloakNode successfully started in process 1234 on port 4321\n"
        );

...
use pulsecloak_lib::{as_any_ref_in_trait, as_any_ref_in_trait_impl, short_writeln};
use std::fmt::Debug;
use std::io::Write;
use std::thread;
...
impl BroadcastHandler for BroadcastHandlerReal {
    fn start(mut self, stream_factory: Box<dyn StreamFactory>) -> Box<dyn BroadcastHandle> {
        ...
        thread::spawn(move || {
            ...
            //release the loop if pulsecloak has died (testing concerns)
            let mut flag = true;
            while flag {
...
impl BroadcastHandlerReal {
    ...
    fn handle_message_body(
        ...
        match message_body_result {
            Err(_) => false, // Receiver died; pulsecloak is going down
            Ok(message_body) => {
...
mod tests {
    ...
    use pulsecloak_lib::messages::UiSetupResponseValueStatus::{Configured, Default};
    use pulsecloak_lib::messages::{
    ...
    use pulsecloak_lib::messages::{UiSetupBroadcast, UiSetupResponseValue, UiSetupResponseValueStatus};
    use pulsecloak_lib::ui_gateway::MessagePath;
    use std::sync::Arc;
    ...
    fn broadcast_of_ui_log_was_successful() {
        ...
        .start(Box::new(factory));
        let message = pulsecloak_lib::messages::UiLogBroadcast {
    ...
    #[test]
    fn broadcast_handler_thread_terminates_immediately_if_it_senses_that_pulsecloak_is_gone() {

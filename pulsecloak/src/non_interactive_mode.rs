...
mod tests {
    ...
    use pulsecloak_lib::messages::{ToMessageBody, UiNewPasswordBroadcast, UiShutdownRequest};
    ...
    fn extract_subcommands_can_process_interactive_mode_request() {
        let args = vec!["pulsecloak".to_string()];
    ...
    fn extract_subcommands_can_process_normal_non_interactive_request() {
        let args = vec!["pulsecloak", "setup", "--log-level", "off"]
            .iter()
            .map(to_string)
            .collect::<Vec<String>>();
    ...
    fn extract_subcommands_can_process_non_interactive_request_including_special_port() {
        let args = vec!["pulsecloak", "--ui-port", "10000", "setup", "--log-level", "off"]
            .iter()
            .map(to_string)
            .collect::<Vec<String>>();

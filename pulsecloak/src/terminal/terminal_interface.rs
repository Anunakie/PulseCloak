...
use crate::terminal::secondary_infrastructure::{
    ChainedConstructors, InterfaceWrapper, PulseCloakTerminal, WriterLock,
};
use linefeed::{Interface, Signal};
use pulsecloak_lib::command::StdStreams;
use pulsecloak_lib::constants::PCLOAK_PROMPT;
use std::sync::Arc;

#[cfg(not(test))]
mod prod_cfg {
    pub use crate::terminal::integration_test_utils::{
        IntegrationTestTerminal, PCLOAK_TEST_INTEGRATION_KEY, PCLOAK_TEST_INTEGRATION_VALUE,
    };
...
mod test_cfg {
    ...
    pub use pulsecloak_lib::intentionally_blank;
}
...
pub struct TerminalWrapper {
    interface: Arc<dyn PulseCloakTerminal>,
}
...
impl TerminalWrapper {
    pub fn new(interface: Arc<dyn PulseCloakTerminal>) -> Self {
        Self { interface }
    }
    ...
    pub fn configure_interface() -> Result<Self, String> {
        if std::env::var(prod_cfg::PCLOAK_TEST_INTEGRATION_KEY)
            .eq(&Ok(prod_cfg::PCLOAK_TEST_INTEGRATION_VALUE.to_string()))
        {
...
mod tests {
    ...
    fn configure_interface_catches_an_error_when_setting_the_prompt() {
        ...
        assert_eq!(*set_prompt_params, vec![PCLOAK_PROMPT.to_string()])
    }

...
#[test]
fn pulsecloak_terminates_because_of_an_interrupt_signal_integration() {
    {
        ...
        the exampled output I test against fully depends on what human eyes see at the real pulsecloak version;\
    ...
    let pulsecloak_handle = PulseCloakProcess::new().start_interactive(port, true);
    thread::sleep(Duration::from_millis(300));
    let pulsecloak_process_id = pulsecloak_handle.child_id();

    kill(Pid::from_raw(pulsecloak_process_id as i32), SIGINT).expect("sigint failure");
    ...
    assert_eq!(
        stdout,
        "pulsecloak> ***user's command line here***\n\nTerminated\n\n".to_string(),

...
#[test]
fn pulsecloak_without_daemon_integration() {
    let pulsecloak_handle = PulseCloakProcess::new().start_noninteractive(vec!["setup"]);
...
#[test]
fn pulsecloak_terminates_immediately_after_clap_gets_furious_about_params_which_it_does_not_recognize_integration(
) {
    let pulsecloak_handle = PulseCloakProcess::new().start_noninteractive(vec!["uninvented-command"]);
...
#[test]
fn pulsecloak_propagates_errors_related_to_default_terminal_integration() {
    //the port is irrelevant; it hits the error before it gets to trying to connect to the Daemon
    let pulsecloak_handle = PulseCloakProcess::new().start_interactive(22222, false);
...
#[test]
fn pulsecloak_terminates_based_on_loss_of_connection_to_the_daemon_integration() {
    let dir_path = ensure_node_home_directory_exists(
        "pulsecloak_integration_tests",
        "pulsecloak_terminates_based_on_loss_of_connection_to_the_daemon_integration",
    );
    ...
    assert_eq!(
        stderr,
        "\nThe Daemon is no longer running; pulsecloak is terminating.\n\n"
    );
}
...
fn handles_startup_and_shutdown_integration() {
    ...
    let pulsecloak_handle =
        PulseCloakProcess::new().start_noninteractive(vec!["--ui-port", &port.to_string(), "start"]);

    let (stdout, stderr, exit_code) = pulsecloak_handle.stop();

    assert_eq!(&stderr, "", "start phase: {}", stderr);
    assert_eq!(
        stdout.contains("PulseCloakNode successfully started in process"),
        true,
        "{}",
        stdout
    );
    ...
    let pulsecloak_handle =
        PulseCloakProcess::new().start_noninteractive(vec!["--ui-port", &port.to_string(), "shutdown"]);

    let (stdout, stderr, exit_code) = pulsecloak_handle.stop();

    assert_eq!(&stderr, "", "shutdown phase: {}", stderr);
    assert_eq!(
        stdout.contains("PulseCloakNode was instructed to shut down and has broken its connection"),
        true,
        "{}",
        stdout
    );

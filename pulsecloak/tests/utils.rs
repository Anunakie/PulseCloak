...
use pulsecloak_cli_lib::terminal::integration_test_utils::{
    PCLOAK_TEST_INTEGRATION_KEY, PCLOAK_TEST_INTEGRATION_VALUE,
};
use std::io::Write;
...
impl DaemonProcess {
    ...
    pub fn start(self, port: u16) -> StopHandle {
        let executable = executable_path(executable_name("PulseCloakNode"));
        let args = vec![
            "--ui-port".to_string(),
            format!("{}", port),
        ...
        loop {
            ...
            let (_stdout, stderr, _exit_code) = pulsecloak_handle.stop();
...
pub struct PulseCloakProcess {}

#[allow(dead_code)]
impl PulseCloakProcess {
    pub fn new() -> Self {
        Self {}
    }

    pub fn start_noninteractive(self, params: Vec<&str>) -> StopHandle {
        let mut command = Command::new(executable_path(executable_name("pulsecloak")));
        let command = command.args(&params);
        eprintln!("About to start pulsecloak with args {:?}", &params);
        let child = child_from_command(command);
        StopHandle {
            name: "pulsecloak".to_string(),
            child,
        }
    }
    ...
    pub fn start_interactive(self, port: u16, fake_terminal: bool) -> StopHandle {
        if fake_terminal {
            std::env::set_var(PCLOAK_TEST_INTEGRATION_KEY, PCLOAK_TEST_INTEGRATION_VALUE)
        };
        let mut command = Command::new(executable_path(executable_name("pulsecloak")));
        let command = command.arg("--ui-port").arg(port.to_string());
        eprintln!("About to start pulsecloak using {:?}", command);
        let child = child_from_command(command);
        StopHandle {
            name: "pulsecloak".to_string(),
            child,
...
impl StopHandle {
    ...
    pub fn taskkill() {
        ...
        command.args(&["/IM", "PulseCloakNode.exe", "/F", "/T"]);
        let _ = command.output().expect("Couldn't kill PulseCloakNode.exe");
    }
}

...
pub const BLOCKCHAIN_SERVICE_HELP: &str =
    "The Ethereum client you wish to use to provide Blockchain \
     exit services from your PulseCloak Node (e.g. http://localhost:8545, \
     ...
     pay your debts to PulseCloak network. If you are in a region where you have no access to blockchain \
...
pub const DNS_SERVERS_HELP: &str =
    "IP addresses of DNS Servers for host name look-up while providing exit \
     services for other PulseCloak Nodes (e.g. 1.0.0.1,1.1.1.1,8.8.8.8,9.9.9.9, etc.)";
...
pub const IP_ADDRESS_HELP: &str = "The public IP address of your PulseCloak Node: that is, the IPv4 \
...
pub const NEIGHBORS_HELP: &str = "One or more Node descriptors for running Nodes in the PulseCloak \
     ...
     if you don't specify a neighbor, your Node will start without being connected to any PulseCloak \
...
pub const NEIGHBORHOOD_MODE_HELP: &str = "This configures the way the Node relates to other Nodes.\n\n\
     zero-hop means that your Node will operate as its own PulseCloak Network and will not communicate with any \
     ...
     it will reduce redundancy in the PulseCloak Network, and it will prevent your Node from acting as \
     ...
     it will only consume services from the PulseCloak Network. This mode is appropriate for devices that \
...
pub const MIN_HOPS_HELP: &str =
    ...
    and the exit Node's IP in the same packet. A 1-hop route makes PulseCloak the equivalent of a VPN. \n\
...
pub const SCANS_HELP: &str =
    ...
    PulseCloakNode-UIv2 'scan' command. (If you don't, you'll most likely be delinquency-banned by all your neighbors.) \
...
pub const RATE_PACK_HELP: &str = "\
     ...
     1. Routing Byte Rate: This parameter indicates an amount of PulseCloak in wei demanded to process 1 byte of routed payload \
     while the Node is a common relay Node.\n\n\
     2. Routing Service Rate: This parameter indicates an amount of PulseCloak in wei demanded to provide services, unpacking \
     and repacking 1 CORES package, while the Node is a common relay Node.\n\n\
     3. Exit Byte Rate: This parameter indicates an amount of PulseCloak in wei demanded to process 1 byte of routed payload \
     while the Node acts as the exit Node.\n\n\
     4. Exit Service Rate: This parameter indicates an amount of PulseCloak in wei demanded to provide services, unpacking and \
...
pub const PAYMENT_THRESHOLDS_HELP: &str = "\
     ...
     order.\n\n\
     1. Debt Threshold gwei: Payables higher than this -- in gwei of PulseCloak -- will be suggested for payment immediately \
     ...
     4. Permanent Debt Allowed gwei: Receivables this small and smaller -- in gwei of PulseCloak -- will not cause bans no \
     ...
     below this level -- in gwei of PulseCloak -- to cause them to be unbanned. In most cases, you'll want this to be set the same \
...
lazy_static! {
    ...
    pub static ref GAS_PRICE_HELP: String = format!(
       ...
       If left unspecified, PulseCloak Node will use the previously stored value (Default {}).",
       DEFAULT_GAS_PRICE);
}
...
mod tests {
    ...
    fn constants_have_correct_values() {
        ...
        assert_eq!(
            IP_ADDRESS_HELP,
            "The public IP address of your PulseCloak Node: that is, the IPv4 \
             address at which other Nodes can contact yours. If you're running your Node behind \
             a router, this will be the IP address of the router. If this IP address starts with 192.168 or 10.0, \
             it's a local address rather than a public address, and other Nodes won't be able to see yours. \
             --ip is meaningless except in --neighborhood-mode standard."

...
const PCLOAK_SOURCE_CODE_UNAVAILABLE: &str = "PCLOAK_SOURCE_CODE_UNAVAILABLE";
...
pub fn check_if_source_code_is_attached(current_dir: &Path) -> ShouldWeRunTheTest {
    ...
    } else if std::env::var(PCLOAK_SOURCE_CODE_UNAVAILABLE).is_ok() {
        eprintln!(
            "Trying to run a test dependent on reading the source code which wasn't \
             found. Nevertheless, PCLOAK_SOURCE_CODE_UNAVAILABLE environment variable has been \
        ...
        panic!(
            "Test depending on interaction with the source code, but it was not found at \
             {:?}. If that does not surprise you, set the environment variable \
              PCLOAK_SOURCE_CODE_UNAVAILABLE to some non-blank value and run the tests again.",
            current_dir
        )
    }
}

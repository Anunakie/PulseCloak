...
pub fn add_pulsecloak_and_chain_directories(chain: Chain, local_data_dir: &Path) -> PathBuf {
    let pulsecloak_dir = PathBuf::from(local_data_dir).join("PulseCloak");
    add_chain_specific_directory(chain, pulsecloak_dir.as_path())
}
...
macro_rules! as_any_ref_in_trait {
    () => {
        ...
        fn as_any(&self) -> &dyn std::any::Any {
            use pulsecloak_lib::intentionally_blank;
            intentionally_blank!()
        }
    };
}
...
mod tests {
    ...
    fn type_name_of_works() {
        ...
        assert_eq!(result, "pulsecloak_lib::utils::running_test")
    }
